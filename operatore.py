...
def _current_db() -> str:
    # No external DB. Return fallback schema name.
    return FALLBACK_SCHEMA

@st.cache_data(ttl=60, show_spinner=False)
def _db_version(schema: str) -> str:
    # provide a transient vkey based on time
...
def _load_table(schema: str, table: str, limit: int, vkey: str) -> pd.DataFrame:
    ...
    Load table data using (in order):
      1) injected get_table / tables dict
...
def _to_datetime_fast(s: pd.Series) -> pd.Series:
    """Parser deterministico:
       - 'dd/mm/yyyy' o 'dd/mm/yyyy hh:mm[:ss]' -> dayfirst
       - 'yyyy-mm-dd' o 'yyyy-mm-dd hh:mm[:ss]' -> YMD
       - altro -> tentativo singolo; se fallisce -> NaT
    """
...
@st.cache_data(ttl=60, show_spinner=False)
def _monthly_by_operator(schema_now: str, vkey: str,
    ...
    # aggrego PER mese, operatore, attività (non sommo su attività!)
...
def render() -> None:
    ...
    mesi_opts, att_opts, _ = _collect_filter_options(schema_now, vkey)
    c1, c2 = st.columns(2)
    with c1:
    ...
    dfm_all = _monthly_by_operator(schema_now, vkey, mesi_sel, att_sel)
    if dfm_all.empty:
